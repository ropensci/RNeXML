...
is provided for the R environment through the `rrdf` package `r citep(citation("rrdf"))`,
...
tmp <- tempfile()  # rrdf requires a file name, so we must write the XML out first
saveXML(rdf, tmp) 
graph <- load.rdf(tmp)
```
...
interoperability difficult `r citep("Vos NeXML 2012")`.  
...
write.bibtex(file="components/sup-references.bib")
unlink("simmap.xml")
unlink("birds.xml")
unlink("geospiza.xml")
unlink(tmp)
