...
evolutionary informatics). For instance, through the use of semantics we can
expect the computer to reason about taxonomic concepts
(as we illustrate in the supplement). 
<!-- Sorry, just trying to contrast what it means to make a 'semantic' query 
vs a non-semantic one.  Most R users won't be familiar with SQL queries either anyway.  Let me know if this is better.-->
...
phylogenies, character matrices, and metadata, respectively. Note that
unlike NeXML or NEXUS, the `phylo` object stores taxonomic information
only as tip labels and not as a separate block of data.
...
As for other R packages, prior to using `RNeXML`, the package needs to
be installed and loaded. The current stable version can be installed from
the CRAN library[^1]:
...
package, containing two small phylogenetic trees.[^2]
...
type `nexml`. This object type is a representation of the NeXML
...
As an example for leveraging the wealth of resources in R
interoperating with the `ape::phylo` type, we use `ape`'s standard
plotting function to plot the first tree in the list of trees:
...
`ape::multiPhylo` objects, reflecting the nested structure of NeXML) is desirable.
`RNeXML` also contains a method `get_trees()`, which
...
Because the example file supplied with the package contains two
phylogenies in a single `trees` block, in this case the return type of both
phylogeny accessor methods is the same:
...
As mentioned above, the `nexml_read()` method allows specifying the
NeXML document to read not only as the name of a local file, but also
as the URL of a document on the web. If the location parameter is
recognized as a URL, the method will automatically download the
...
NeXML format on the web, which includes the content of the
phylogenetic data repository TreeBASE [@Piel_2002]. For
example, the following plots a tree in TreeBASE:

```{r eval=FALSE}
tb_nex <- nexml_read("https://raw.github.com/TreeBASE/supertreebase/master/data/treebase/S100.xml")
...
NeXML documents can contain character or trait data in the form of
matrices. A data matrix can be of molecular (for molecular sequence
alignments), discrete (for most morphological character data), or
continuous type (for many trait data). Character data, especially
molecular and morphological matrices, are often used for phylogenetic
tree reconstruction, but they are also a common input for the many
comparative phylogenetic analysis methods available in R. The method
`get_characters()` obtains character data from a `nexml` object, and
returns them as a standard `data.frame` R object with columns as
characters and rows as taxa. The following example, which uses a file
that comes with `RNeXML`, demonstrates this:
...
Observe that `RNeXML` has automatically merged the different NeXML character
blocks, (representing continuous and discrete characters respectively) 
into a single data frame with appropriate column classes. 
To allow strict validation of data types, the NeXML schema requires
continuous and discrete character data to be separated into different
"characters" blocks. Since the `data.frame` data structure in R has no
such constraints, the `get_characters()` method will combine such
blocks as separate columns into a single `data.frame` object, provided
they correspond to the same taxa. Otherwise, a list of `data.frame`s
is returned, with list elements corresponding to characters
blocks. Similar to the distinction between `get_trees()` and
`get_trees_list()`, there is also a method `get_characters_list()`,
which always returns a list of `data.frame`s, one for each
character block.


## Writing phylogenetic trees and character data

Aside from providing an easy-to-use interface to data already in
NeXML, `RNeXML` also aims to facilitate broader adoption of the
standard for sharing phylogenetic data, and thus to make it
straightforward to generate them. The method `nexml_write()`, as the
name suggests, generates a NeXML file from its input parameters. In
...
In addition to trees, the `nexml_write()` method also to specify
