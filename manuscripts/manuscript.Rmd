...
research `r citep("10.1371/currents.tol.bf01eff4a6b60ca4825c69293dc59645")`.  



<!-- What do we need to say, if anything, about the current R formats?  about phylobase?  -->

The R language `r citep(citation())` has become an important environment for 
...
`r citep("Chamberlain taxize F1000Research 2013")` to check each taxon label against the NCBI database.
...
explicitly declares the meaning of the annotation. For instance, the title we gave to the `bird.orders`
...
queries, see `r citet("http://www.w3.org/TR/rdf-sparql-query/")` and
`r citet("Hartig Introduction to SPARQL")`.  
