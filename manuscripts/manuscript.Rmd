...
between research groups `r citep(c("Drew PLOS Biology Tree of Life 2013", "10.1186/1756-0500-5-574"))`.  
...
`RNeXML` seeks to provide a seemless way to convert these to and from their 
...
as described in @Vos_2012. <!-- better to cite the W3C's RDFa TR -->
...
the end user. By associating the
taxonomic labels against a specific, centralized database we can catch
some of these inconsistencies and indicate more precisely which classification
...
<!-- does this mean we can use CURIEs whose prefix has not yet been 
bound to a namespace? that would be bad. there should probably be an
...
<!-- I feel like we already covered what dc:title is -->
...
<!-- I think it is unwise to go here: RDF is not structured data -->
...
queries, see `r citet("Hartig Introduction to SPARQL")`.  Meanwhile, we just introduce the syntax
...
<!-- it would be nice if we had a more persuasive example here. directly getting the value
of some predicate can be done the syntax-oriented way by treating the data as XML. what I
think we should be going for here is an example of a slightly more complex query that would
be painful to perform by scanning the XML document. I'm thinking of some sort of join of
multiple predicate values. Nothing great comes to mind yet, though... ->
