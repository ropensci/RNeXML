...
<!-- hmm, taskview citation renders as "'CRAN task view' 2014" intended? -->
...
<!-- Carl, I assume a period goes here, I inserted one -->
...
of the kinds of data annotation that is currently available in R
formats
<!-- R packages, not formats, right? HL -->
...
nex <- nexml_read(f)
```
<!-- The system.file() command is not pretty. Can we not just read in -->
<!-- the file directly? HL -->
...
popular `ape` R package [@Paradis_2004]. --> Although new custom R functions and packages 
...
[TreeBASE](http://treebase.org/) <!-- first mention -->
...
definition of the
NeXML standard, the RNeXML package can successfully read every valid
...
single NeXML file.
<!-- perhaps give a motivating use case for why one might -->
<!-- want to do this? HL -->
