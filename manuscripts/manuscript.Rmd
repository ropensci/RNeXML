...
in the `ape` package [@Paradis2004]), which represents trees alone.
...
which a consistent and predictable return type (namely
`ape::multiPhylo`) is desirable.
<!-- the open (and previously not covered) issue here is what does -->
<!-- get_trees_list() return in the case of multiple trees blocks? HL -->
Following a common design practice
for R packages, `RNeXML` also contains a method `get_trees()`, which
...
introspection commands such as `typeof()`).
...
they correspond to the same taxa.
<!-- does the example data file actually demonstrate this? if not, it -->
<!-- should. HL -->
Otherwise, a list of `data.frame`s
...
library("geiger")
data(geospiza)
nexml_write(trees = geospiza$phy, characters = geospiza$dat,
            file="geospiza.xml")
```
...
<!-- Might be worth adding some brief explanation here as to what -->
<!-- happens internally in such a case. For example, automatic -->
<!-- generation of multiple `otus` blocks? HL --> 
