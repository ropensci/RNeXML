...
NEXUS file counterparts,
<!-- such as what? we haven't talked about metadata yet. HL -->
and users lack a suitable alternative for
...
<!-- such as what? should walk through an example here. HL -->
but inherits the limitations of the NEXUS format.
<!-- how can information inherit the limitations of a serialization -->
<!-- format if it's added to data read from that format? HL -->
<!-- CB: Sorry, was refering to data _not_ read from NEXUS format,
but rather data created in an R format/data-structure.  Information
that is in that R data structure (e.g. a simmap) is lost when using
write.nexus(simmap) -->
...
-->
<!-- I think this is in fact more important than the paragraph above. I.e., -->
<!-- whether exchanging basic comparative data needs another format is -->
<!-- debatable, but that metadata that can hardly be expressed in -->
<!-- current standards does is a clear point. HL -->
...
of the kinds of data annotation that is currently available in data structures
provided by existing R packages
...
[@Xie_2013].  The reader may paste the code blocks
...
A NeXML document can be thought of as having the following components: (1) phylogeny
...
corresponding R object representations, in particular `ape::phylo`,
`data.frame`, and lists for phylogenies, character
matrices, and metadata, respectively.
<!-- I feel the following is a little early - it will likely be over -->
<!-- most most readers' heads. HL
(or for advanced users,
`XMLInternalDocument` and `rrdf` triplestores).
-->
Metadata can provide bibliographic and provenance
information about the study as part of which the phylogeny was generated or
applied (such as authors, citations to publications, timestamps, and even
descriptions of methods). They can also be used to attach annotations to individual
data elements.  We illustrate how metadata annotations can be added to
...
<!-- sounds awkward - need to smoothen the language. HL -->
...
file. <!-- This seems redundant to me to what has already been 
said. HL -->
<!-- CB: Tried to revise what follows to emphasize that we have a new data structure / object class here,
that has unique merits -->
...
<!-- having a function's return type being more or less unpredictable -->
<!-- sounds like a bad design pattern. Is there at least precedence for -->
<!-- this practice in other R packages? HL --> 

<!-- Yes, sadly it is.  Really this is a class definition issue that multiPhylo isn't a
proper generalization of a phylo object, but rather a different class entirely. 
Users who don't like this function can use the get_trees_list() to always be predictable.  -->
...
```
<!-- the example makes me wonder how nexml_read() distinguishes -->
<!-- between a URL and a file name as its argument. HL -->
<!-- just type `nexml_read` and you'll find out ;-).  We use a simple regex  CB -->

Reading phylogenies from TreeBASE into R was already possible using
the R package `treebase` [@Boettiger_2012]. However, because it relied
on the NEXUS parser provided in the `ape` package, and because of
incompatible flavors of the NEXUS standard [@Vos_2012], 
a significant number <!-- State explicitly --> of TreeBASE NEXUS
files could not be read in this way.  Owing to the syntactically strict
definition of the
NeXML standard, the RNeXML package can successfully read every 
NeXML document from TreeBASE. <!-- are there invalid ones as well that -->
<!-- it can't read? the sentence as is suggests so, and so is a little -->
<!-- awkward (it leaves open the possibility that the NEXUS files ape -->
<!-- can't read are now invalid NeXML files. Which is probably not -->
<!-- what you mean to suggest. HL -->
<!-- Correct, there are indeed a few invalid ones (e.g. some are purely
erroneous HTML, see issues on supertreebase repo) which it cannot
parse.  But like you say, no need to confuse the reader.  I've dropped the 
word 'valid'.  -->
...
get_characters(nex)
```
<!-- same as above - why can this not be one call? HL -->
<!-- CB: I've dropped the system.file, even though that is sadly the 
standard way for R to load data that comes with a package but is not in CSV, R, or .rda format.

Indeed `get_characters` could call the file path directly, but there is key
conceptual reason for not doing this.  Note that all the examples call nexml_read to get a nex object
before using get_characters, get_trees, etc.  That is because we really want the user to get used
to manipulating the nexml R data structure, rather than using the external XML file directly.

The functions embody this design principle deep down.  We could have written get_characters etc
in a way that it just yanked the characters out of the XML and left everything else, rather than 
converting the whole NeXML file into a serialized R object stored in memory.  But the latter strategy
gives us a native R object that people can extend, write functions for, and subset and query with 
the standard R syntax.  Without this, the package isn't really providing a native format, and users 
and developers are left on their own to either also work directly with the XML or just convert immediately
into a more limited R format like ape::phylo.  
-->
...
`RNeXML` automatically adds basic metadata by default
<!-- what does "by default" mean here? It doesn't seem like calling -->
<!-- any RNeXML function will add such metadata if it's not there -->
<!-- already? HL -->
<!-- Indeed it does.  We add the time stamp, default license, and user ID even though
     it's not "there" in the data.  -->
such as a time-stamp
of when a tree was produced, and an open data license. The latter by
default is the Creative Commons Zero (CC0) public domain waiver, which
is compatible
with deposition in the Dryad data repository, but can be configured
as a different license if needed. 

To adjust these and other metadata fields commonly recommended to be
included with publicly archived data, `RNeXML` provides the `add_basic_meta()`
...
<!-- I notice that other add_* methods take the nexml object as the -->
<!-- last argument, not the first as seems to be the case here. This -->
<!-- is inconsistent and should be fixed. HL -->

<!-- Not so, the nexml object is the second argument here too.  If no existing 
nexml object is given, the add function starts a new nexml object.  If one is
given, it appends it's first argument onto the existing nexml, as described above.  

Notice that bird.orders is an ape::phylo object, which we are adding to a new 
nexml object.  (R allows for default arguments, the default second argument is 
new("nexml") -->
