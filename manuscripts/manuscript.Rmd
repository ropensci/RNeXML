...
abstract: | 
      ...
      <!-- refer to NeXML as a format and NEXUS as a standard, correct? -->
...
[TreeBASE](http://treebase.org/treebase-web/home.html) <!-- first mention -->
...
NeXML files can also be built up bit by bit using `add_characters()`,
...
`add_basic_meta()` function takes an existing `nexml` object (or will create a new one
...
<!-- the function internally used in taxize_nexml, taxize::get_uid, 
should prompt user to pick which taxon they want, or is that not desired
behavior  -->
...
<!-- hmm, phylobase ref renders for me as 'al. 2014' -->
