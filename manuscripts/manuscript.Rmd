...
extensible, and semantically aware format to import, exchange and archive 
phylogenetic trees, traits, and metadata.
...
for sharing, both between software environments and 
...
(such as a DNA sequence base pair).  This has led many developers to
modify NEXUS in different ways for different needs, with the unfortunate
...
or element lengths.  @Vos_2012 estimates as many as 15% of the NEXUS
files in the CIPRES portal contain unrecoverable but hard to diagnose errors.

NeXML was developed to addresses each of these problems and also open doors
to new opportunities in handling phylogenetic data. First, NeXML is defined
by a precise grammar that can be programmatically **validated**. This helps
ensure that any NeXML file can always be read without errors by any 
NeXML parsing software. Second, NeXML is **extensible**: a user can define
representations of new information (as we will illustrate) without breaking
this compatibility. Third and most importantly, NeXML is **semantically rich**.
The use of formal semantics in NeXML provides a representation of data that
is not only machine-readable, but can also permits machine reasoning about
the data provided (see @Parr2011 for an overview on semantic reasoning in 
evolutionary informatics). Instead of asking the computer to
return "the data on line #93", we can expect the computer to reason about
taxonomic concepts (as we illustrate in the supplement). 

The R language [@R] has become an important environment for phylogenetics
research, with over 30 packages available including several with over 
1000 citations [@taskview]. `RNeXML` makes the large and rapidly growing
archive of richly annotated, NeXML-based phylogenetics data available to
the R user and this powerful ecosystem of R tools for phylogenetics.
Further, R packages and users frequently add their own additional 
information to the phylogenies they work with: annotations of species,
stochastic character maps, trait values, model estimates and parameter 
values. By leveraging the semantic and extensible nature of NeXML, `RNeXML` 
provides R users a platform independent data exchange format to capture,
further annotate, exchange, and preserve this information. With `RNeXML`,
R users and developers can both better contribute to and take advantage 
of the rapidly growing infrastructure of evolutionary informatics [@Parr2011].  
By capturing information already available in the R environment, together
with the appropriate choice of defaults, `RNeXML` can facilitate meeting
the rising bar for best practices in data sharing in phylogenetic 
research [@Cranston_2014] while at the same reducing the burden
on the user.  
...
structures automatically.  We end with an example that uses semantic metadata to
extend the NeXML standard to describe stochastic character mapping: a kind of
data annotation that is currently available in data structures
