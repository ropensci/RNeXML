...
abstract: | 
      ...
      informatics data and metadata exchange formats, offering substantial
      improvements in reliability, extensibility, and richness over
      earlier standards such as NEXUS. In this paper we present the RNeXML
      package which provides a bridge between this rich and reliable
...
such as `ouch` `r citep(citation("ouch"))` and `phylobase` `r citep(citation("phylobase"))` phylogeny
...
These examples use data from the popular phylogenetics software `geiger` `r citep(citation("geiger"))`.  
...
"Resource Description Framework in Annotations", or RDFa `r citep("http://www.w3.org/TR/xhtml-rdfa-primer")`,
...
`r citet("Revell phytools 2011")` describes the `phytools` package for R, which includes
