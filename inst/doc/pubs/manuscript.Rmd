...
2) Adding metadata, including automated addition of metadata
3) 
...
it arbitrarily to meet their needs, making it incompatible.  Cannot
represent horizontal gene transfer, ambiguous use of symbols, cannot be
extended reliably, no validator.  NeXML addresses all of these problems
...
share and archive this. (3) Annotate and extend nexml.
...
the CRAN library using the standard <!-- Actually not yet --> 
...
```{r} 
otu_meta <- get_metadata(S100, level="otu")
otu_meta[1:2]
```
...
utilities for reading, manipulating, and writing `simmap` files.
...
```{r eval = FALSE}
id <- nexml_publish(nexml, visibility = "public", repository="figshare")
```
