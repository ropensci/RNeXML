...
test_that("Parse a complete NeXML file to a single otu", {
  ...
  otu <- as(root[["otus"]][[1]], "nexml:otu")
  expect_that(otu, is_a("nexml:otu"))
...
test_that("Parse a complete NeXML file to trees", {
  trees <- as(root[["trees"]], "nexml:trees")
  expect_that(trees, is_a("nexml:trees"))
  as(trees, "XMLInternalNode")
})

test_that("Parse a complete NeXML file to many otus", {
  otus <- as(root[["otus"]], "nexml:otus")
  expect_that(otus, is_a("nexml:otus"))
