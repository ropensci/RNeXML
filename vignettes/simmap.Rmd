...
**NOTE:** _The code from here to the end of the document no longer works
due to longstanding bugs that recent changes in the `RNeXML` code brought
to the surface. The following code chunks are therefore currently disabled._
...
```{r Figure1, fig.cap="Stochastic character mapping on a phylogeny, as generated by the phytools package after parsing the simmap-extended NeXML.",  eval=FALSE}
library("phytools")
plotSimmap(phy)
```
